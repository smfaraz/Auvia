...
import { 
  ...
  Menu, BarChart3, Pencil, Eye, FileText, UserPlus
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

// Using Design Recipe 1: Technical Dashboard / Data Grid
export const AdminDashboard = () => {
  const [activeTab, setActiveTab] = useState<'leads' | 'jobs' | 'faqs' | 'admins' | 'locations' | 'analytics' | 'applications'>('analytics');
  ...
  const [adminRoleForm, setAdminRoleForm] = useState<'super_admin' | 'center_manager'>('center_manager');
  const [adminCenterForm, setAdminCenterForm] = useState('');
  ...
  useEffect(() => {
    ...
    const unsubscribeLeads = onSnapshot(leadsQuery, 
      (snapshot) => {
        ...
        setSelectedLead(current => {
          ...
          const updated = leadsData.find(l => l.id === current.id);
          return updated || null;
        });
      },
    ...
    const unsubscribeLocations = onSnapshot(locationsQuery, 
      ...
      (error) => console.error("Locations listener permission error:", error)
    );

    const applicationsQuery = query(collection(db, 'job_applications'), orderBy('createdAt', 'desc'));
  ...
  const handleAddAdminSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const emailLower = adminEmailForm.trim().toLowerCase();
    if (!emailLower) return;

    // Check for duplicates
    const isDuplicate = admins.some(a => a.email?.toLowerCase() === emailLower);
    if (isDuplicate) {
    ...
    try {
      ...
      if (adminRoleForm === 'center_manager' && adminCenterForm) {
        adminData.center = adminCenterForm;
      }

      await setDoc(doc(db, 'admins', emailLower), adminData);
      setIsAddAdminModalOpen(false);
      setAdminEmailForm('');
      setAdminCenterForm('');
      showToast('Admin email whitelisted successfully!', 'success');
    } catch (error) {
      console.error('Error whitelisting admin:', error);
      showToast('Failed to whitelist admin.', 'error');
    }
  };

  const handleApplicationStatusUpdate = async (appId: string, newStatus: string) => {
    try {
  ...
  const tourLeads = leads.filter(l => l.status === 'tour_scheduled').length;
  ...
  const totalCenters = locations.length;

  // Lead sources breakdown
  const sourcesCount = leads.reduce((acc: any, curr: any) => {
  ...
  // Leads by center
  const centerLeadsCount = leads.reduce((acc: any, curr: any) => {
    const centerName = curr.center || 'Unassigned';
        ...
        <div className="p-8 pb-12">
          ...
          <nav className="space-y-2">
            {[
              ...
              { id: 'applications', icon: FileText, label: 'Applications', badge: applications.filter(app => app.status === 'applied').length },
              ...
              >
                ...
                {item.badge !== undefined && item.badge > 0 && (
                  <span className="ml-auto bg-brand-peach text-brand-ink text-[10px] font-bold px-2 py-0.5 rounded-full border border-brand-ink/10">
                    {item.badge}
                  </span>
                )}
              </button>
      ...
      <main className="flex-1 flex flex-col h-screen min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-10 relative">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <h3 className="text-small-heading font-kids font-bold text-brand-ink capitalize">
              {activeTab === 'analytics' ? 'Insights Overview' : activeTab === 'applications' ? 'Recruitment Dashboard' : `${activeTab} Management`}
            </h3>
            {activeTab !== 'leads' && activeTab !== 'admins' && activeTab !== 'analytics' && activeTab !== 'applications' && (
              <button 
                onClick={() => {
                  setEditingItem(null);
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'analytics' && (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <div className="bg-white border border-[#141414] rounded-none shadow-[4px_4px_0px_#141414] p-6 flex items-start justify-between">
                    <div className="space-y-2">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-brand-sage">Total Interest Leads</span>
                      <h4 className="text-3xl font-kids font-bold text-brand-ink">{totalLeads}</h4>
                      <div className="flex flex-wrap gap-2 pt-1">
                        <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-brand-mint text-brand-teal border border-brand-teal/15">{newLeads} New</span>
                        <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-brand-sky text-brand-teal border border-brand-teal/15">{contactedLeads + tourLeads} Active</span>
                      </div>
                    </div>
                    <div className="w-12 h-12 rounded-xl bg-brand-mint/50 text-brand-teal flex items-center justify-center border border-brand-teal/20 shrink-0">
                      <Users size={20} />
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-[#141414] rounded-none shadow-[4px_4px_0px_#141414] p-6 flex items-start justify-between">
                    <div className="space-y-2">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-brand-sage">Enrollment Rate</span>
                      <h4 className="text-3xl font-kids font-bold text-[#0f766e]">{conversionRate}%</h4>
                      <p className="text-[10px] font-medium text-brand-sage leading-normal">
                        {enrolledLeads} of {totalLeads} interest leads enrolled
                      </p>
                    </div>
                    <div className="w-12 h-12 rounded-xl bg-brand-peach/50 text-brand-teal flex items-center justify-center border border-brand-teal/20 shrink-0">
                      <CheckCircle size={20} />
                    </div>
                  </div>

                  {/* KPI Card 3: Job Applications Received */}
                  <div className="bg-white border border-[#141414] rounded-none shadow-[4px_4px_0px_#141414] p-6 flex items-start justify-between">
                    <div className="space-y-2">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-brand-sage">Job Applications</span>
                      <h4 className="text-3xl font-kids font-bold text-brand-ink">{applications.length} Received</h4>
                      <div className="flex flex-wrap gap-2 pt-1">
                        <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-brand-mint text-brand-teal border border-brand-teal/15">{applications.filter(a => a.status === 'applied').length} New</span>
                        <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-brand-sky text-brand-teal border border-brand-teal/15">{applications.filter(a => a.status === 'interviewing').length} In Interview</span>
                      </div>
                    </div>
                    <div className="w-12 h-12 rounded-xl bg-brand-lavender/60 text-brand-teal flex items-center justify-center border border-brand-teal/20 shrink-0">
                      <FileText size={20} />
                    </div>
                  </div>

                  {/* KPI Card 4: Total Care Centers */}
                  <div className="bg-white border border-[#141414] rounded-none shadow-[4px_4px_0px_#141414] p-6 flex items-start justify-between">
                    <div className="space-y-2">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-brand-sage">Registered Centers</span>
                      <h4 className="text-3xl font-kids font-bold text-brand-ink">{totalCenters}</h4>
                      <p className="text-[10px] font-medium text-brand-sage">
                        Auvia Care Sanctuaries
                      </p>
                    </div>
                    <div className="w-12 h-12 rounded-xl bg-brand-sky/60 text-brand-teal flex items-center justify-center border border-brand-teal/20 shrink-0">
                      <MapPin size={20} />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  {/* Lead Funnel Widget */}
                  <div className="bg-white border border-[#141414] rounded-none shadow-[4px_4px_0px_#141414] p-6 lg:p-8 space-y-6">
                    <div className="border-b border-[#141414]/10 pb-4 flex justify-between items-center">
                      <h4 className="font-kids font-bold text-brand-ink text-base">Client Conversion Funnel</h4>
                      <span className="text-[10px] font-bold uppercase tracking-widest text-[#0f766e] bg-brand-mint px-2 py-0.5 rounded border border-brand-teal/15">Real-time status</span>
                    </div>

                    <div className="space-y-4">
                      {[
                        { status: 'new', label: 'New Inquiry', count: newLeads },
                        { status: 'contacted', label: 'Contacted / Follow-Up', count: contactedLeads },
                        { status: 'tour_scheduled', label: 'Tour Scheduled', count: tourLeads },
                        { status: 'enrolled', label: 'Enrolled Client', count: enrolledLeads }
                      ].map((step, idx) => {
                        const pct = totalLeads > 0 ? Math.round((step.count / totalLeads) * 100) : 0;
                        return (
                          <div key={step.status} className="space-y-1">
                            <div className="flex justify-between items-center text-xs">
                              <div className="flex items-center gap-2 font-bold font-kids text-brand-ink">
                                <span className="w-5 h-5 rounded-full bg-[#141414] text-white flex items-center justify-center text-[10px] font-mono">{idx + 1}</span>
                                {step.label}
                              </div>
                              <span className="font-mono text-brand-sage font-bold">{step.count} ({pct}%)</span>
                            </div>
                            <div className="w-full h-3 bg-brand-cream border border-[#141414] rounded-none overflow-hidden">
                              <div className="h-full bg-brand-teal transition-all duration-500" style={{ width: `${pct}%` }} />
                            </div>
                          </div>
                        );
                      })}
                  ...
                  {/* Lead Sources & Center Distributions */}
                  <div className="bg-white border border-[#141414] rounded-none shadow-[4px_4px_0px_#141414] p-6 lg:p-8 space-y-6">
                    <div className="border-b border-[#141414]/10 pb-4">
                      <h4 className="font-kids font-bold text-brand-ink text-base">Inquiry Channels</h4>
                    </div>

                    <div className="space-y-4">
                      {[
                        { key: 'contact_page', label: 'Contact Form Submissions' },
                        { key: 'home_interest_form', label: 'Home Page Interest forms' },
                        { key: 'insurance_inquiry_form', label: 'Insurance Inquiry Forms' }
                      ].map((src) => {
                        const count = sourcesCount[src.key] || 0;
                        const pct = totalLeads > 0 ? Math.round((count / totalLeads) * 100) : 0;
                        return (
                          <div key={src.key} className="space-y-1">
                            <div className="flex justify-between items-center text-xs">
                              <span className="font-kids font-bold text-brand-ink">{src.label}</span>
                              <span className="font-mono text-brand-sage font-bold">{count} ({pct}%)</span>
                            </div>
                            <div className="w-full h-2 bg-brand-cream border border-[#141414] rounded-none overflow-hidden">
                              <div className="h-full bg-brand-teal-light transition-all duration-500" style={{ width: `${pct}%` }} />
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div className="pt-4 border-t border-[#141414]/10">
                      <h5 className="font-kids font-bold text-brand-ink text-sm mb-3">Leads by Preferred Center</h5>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[160px] overflow-y-auto pr-1">
                        {Object.entries(centerLeadsCount).map(([center, count]: [string, any]) => (
                          <div key={center} className="flex justify-between items-center p-3 bg-brand-cream border border-[#141414] rounded-xl text-xs font-bold">
                            <span className="truncate max-w-[120px] text-brand-ink capitalize">{center}</span>
                            <span className="px-2 py-0.5 bg-brand-mint text-brand-teal border border-brand-teal/20 rounded font-mono">
                              {count}
                            </span>
                          </div>
                        ))}
                        {Object.keys(centerLeadsCount).length === 0 && (
                          <div className="col-span-2 text-center text-xs text-brand-sage italic py-2">
                      ...
                      <div className="col-span-2">
                        ...
                        <div className="flex items-center gap-3">
                          <button 
                            ...
                            className="flex-1 lg:flex-none p-3 lg:p-2 border border-[#141414] lg:border-current rounded-xl hover:bg-white hover:text-brand-ink transition-all flex justify-center items-center"
                            title="View Details"
                          >
                            <Eye size={14} />
                          </button>
            ...
            {activeTab === 'applications' && (
              ...
              >
                {/* Headers */}
                <div className="hidden lg:grid grid-cols-12 border-b border-[#141414] bg-[#F8F7F4] p-4">
                  <div className="col-span-3 font-serif italic text-[11px] uppercase tracking-wider opacity-50">Candidate</div>
                  <div className="col-span-3 font-serif italic text-[11px] uppercase tracking-wider opacity-50">Position</div>
                  <div className="col-span-2 font-serif italic text-[11px] uppercase tracking-wider opacity-50">Resume</div>
                  <div className="col-span-2 font-serif italic text-[11px] uppercase tracking-wider opacity-50">Status</div>
                  <div className="col-span-2 font-serif italic text-[11px] uppercase tracking-wider opacity-50 text-right">Actions</div>
                </div>

                {/* Rows */}
                <div className="divide-y divide-[#141414]">
                  {loading ? (
                    <div className="p-20 text-center text-gray-400 font-mono text-sm">LOADING_APPLICATIONS...</div>
                  ) : filteredApplications.length === 0 ? (
                    <div className="p-20 text-center text-gray-400 font-mono text-sm">NO_APPLICATIONS_FOUND</div>
                  ) : filteredApplications.map((app) => (
                    <div 
                      key={app.id}
                      className="flex flex-col lg:grid lg:grid-cols-12 p-5 lg:p-6 hover:bg-[#141414] hover:text-white transition-all group gap-4 lg:gap-0"
                    >
                      {/* Contact */}
                      <div className="col-span-3">
                        <div className="lg:hidden text-[9px] font-bold text-brand-teal uppercase tracking-[0.2em] mb-2">Candidate</div>
                        <div className="font-bold text-base lg:text-sm mb-1">{app.firstName} {app.lastName}</div>
                        <div className="font-mono text-[10px] opacity-70 break-all">{app.email}</div>
                        <div className="font-mono text-[10px] opacity-70 mt-1">{app.phone}</div>
                      </div>

                      {/* Position details */}
                      <div className="col-span-3">
                        <div className="lg:hidden text-[9px] font-bold text-brand-teal uppercase tracking-[0.2em] mb-2">Position</div>
                        <div className="text-sm font-medium">{app.jobTitle}</div>
                        <div className="text-[10px] opacity-60">Submitted: {app.createdAt ? (app.createdAt.seconds ? new Date(app.createdAt.seconds * 1000).toLocaleDateString() : new Date(app.createdAt).toLocaleDateString()) : 'N/A'}</div>
                      </div>

                      {/* Resume */}
                      <div className="col-span-2">
                        <div className="lg:hidden text-[9px] font-bold text-brand-teal uppercase tracking-[0.2em] mb-2">Resume link</div>
                        <a 
                          href={app.resumeUrl} 
                          target="_blank" 
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1.5 px-3 py-1 bg-brand-mint text-brand-teal text-[10px] font-bold tracking-wider rounded border border-brand-teal/15 hover:bg-white hover:text-brand-ink transition-colors uppercase"
                        >
                      ...
                      {/* Status select */}
                      <div className="col-span-2">
                        <div className="lg:hidden text-[9px] font-bold text-brand-teal uppercase tracking-[0.2em] mb-2">Recruitment Pipeline</div>
                        <select 
                          value={app.status}
                      ...
                      <div className="col-span-2">
                        ...
                        <div className="flex items-center justify-end gap-3 font-sans">
                          {app.message && (
                            <button 
                              onClick={() => alert(`Cover Note from ${app.firstName}:\n\n${app.message}`)}
                              className="flex-1 lg:flex-none p-3 lg:p-2 border border-[#141414] lg:border-current rounded-xl hover:bg-white hover:text-brand-ink transition-all flex justify-center items-center"
                      ...
                      <div className="col-span-3 flex flex-col gap-1 items-start justify-center">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {admin.status === 'pending' && (
                            <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 bg-brand-peach text-brand-ink border border-brand-ink/10 rounded-full">
                              PENDING
                            </span>
                          )}
                        </div>
                        {admin.center && (
                          <span className="text-[10px] opacity-60 font-bold capitalize">
                            Center: {admin.center}
                          </span>
                        )}
                      </div>
          ...
          {isModalOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 lg:p-0">
              ...
              >
                ...
                <h4 className="text-section font-kids font-bold mb-8">
        ...
        <AnimatePresence>
          {selectedLead && (
            <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 lg:p-0">
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              ...
              <motion.div 
                ...
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.95, y: 20 }}
                className="relative w-full max-w-4xl bg-white rounded-none border border-[#141414] shadow-2xl p-6 md:p-10 overflow-hidden max-h-[90vh] flex flex-col"
              >
                <button onClick={() => setSelectedLead(null)} className="absolute top-6 right-6 p-2 text-[#141414] hover:bg-[#141414] hover:text-white rounded-full transition-all">
                  <X size={20} />
                </button>
                
                <h4 className="text-section font-kids font-bold mb-6 text-brand-ink">
                  Lead Profile: {selectedLead.firstName} {selectedLead.lastName}
                </h4>

                <div className="flex-1 overflow-y-auto space-y-8 pr-2">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Information panel */}
                    <div className="space-y-4 bg-brand-cream border border-[#141414] p-6 rounded-none">
                      <h5 className="font-kids font-bold text-sm text-brand-teal border-b border-brand-teal/10 pb-2">Parent & Child Details</h5>
                      <div className="space-y-3 text-xs">
                        <div className="grid grid-cols-3">
                          <span className="text-brand-sage font-bold">Email:</span>
                          <span className="col-span-2 font-mono break-all text-brand-ink">{selectedLead.email}</span>
                        </div>
                        <div className="grid grid-cols-3">
                          <span className="text-brand-sage font-bold">Phone:</span>
                          <span className="col-span-2 text-brand-ink">{selectedLead.phone}</span>
                        </div>
                        <div className="grid grid-cols-3">
                          <span className="text-brand-sage font-bold">Child Name:</span>
                          <span className="col-span-2 text-brand-ink">{selectedLead.childName || 'N/A'}</span>
                        </div>
                        <div className="grid grid-cols-3">
                          <span className="text-brand-sage font-bold">Child DOB:</span>
                          <span className="col-span-2 text-brand-ink">{selectedLead.childDob || 'N/A'}</span>
                        </div>
                        <div className="grid grid-cols-3">
                          <span className="text-brand-sage font-bold">Insurance:</span>
                          <span className="col-span-2 text-brand-ink font-bold">{selectedLead.insuranceProvider || 'N/A'}</span>
                        </div>
                        <div className="grid grid-cols-3">
                          <span className="text-brand-sage font-bold">Center Prefer:</span>
                          <span className="col-span-2 text-brand-ink capitalize">{selectedLead.center || 'N/A'}</span>
                        </div>
                        <div className="grid grid-cols-3">
                          <span className="text-brand-sage font-bold">Zip Code:</span>
                          <span className="col-span-2 text-brand-ink font-mono">{selectedLead.zipCode || selectedLead.zip || 'N/A'}</span>
                        </div>
                        <div className="grid grid-cols-3">
                          <span className="text-brand-sage font-bold">SMS Consent:</span>
                          <span className="col-span-2">
                            {selectedLead.smsConsent ? (
                              <span className="text-green-600 font-bold">Granted</span>
                            ) : (
                              <span className="text-red-500 font-bold">Pending / Declined</span>
                            )}
                          </span>
                        </div>
                        <div className="grid grid-cols-3">
                          <span className="text-brand-sage font-bold">Source:</span>
                          <span className="col-span-2 text-brand-ink font-mono uppercase text-[10px] tracking-wider">{selectedLead.source || 'N/A'}</span>
                        </div>
                        <div className="grid grid-cols-3">
                          <span className="text-brand-sage font-bold">Created:</span>
                          <span className="col-span-2 text-brand-ink font-mono">
                    ...
                    {/* Message panel */}
                    <div className="space-y-4 bg-brand-cream border border-[#141414] p-6 rounded-none flex flex-col">
                      <h5 className="font-kids font-bold text-sm text-brand-teal border-b border-brand-teal/10 pb-2">Parent Message</h5>
                      <div className="flex-1 text-xs text-brand-sage font-medium italic overflow-y-auto leading-relaxed max-h-[200px]">
                        {selectedLead.message ? `"${selectedLead.message}"` : 'No message submitted by parent.'}
                      </div>
                    </div>
                  </div>

                  {/* Notes CRM section */}
                  <div className="border border-[#141414] p-6 rounded-none space-y-6">
                    <h5 className="font-kids font-bold text-sm text-brand-ink flex items-center justify-between border-b border-[#141414]/10 pb-2">
                      <span>Internal Follow-Up Timeline</span>
                      <span className="text-[10px] font-mono opacity-50">Saved in Lead Doc</span>
                    </h5>
                    ...
                    <div className="space-y-4 max-h-[250px] overflow-y-auto pr-1">
                      {selectedLead.notes && selectedLead.notes.length > 0 ? (
                        selectedLead.notes.map((note: any, index: number) => (
                          <div key={index} className="flex gap-4 items-start text-xs border-l-2 border-brand-teal pl-4 relative">
                            <div className="absolute w-2 h-2 rounded-full bg-brand-teal -left-[5px] top-1.5" />
                            <div className="space-y-1">
                              <div className="flex items-center gap-2 text-brand-sage">
                                <span className="font-bold text-brand-ink">{note.author}</span>
                                <span>•</span>
                                <span>{new Date(note.createdAt).toLocaleString()}</span>
                              </div>
                              <p className="text-brand-sage font-medium whitespace-pre-wrap">{note.text}</p>
                            </div>
                          </div>
                        ))
                      ) : (
                        <p className="text-xs text-brand-sage italic text-center py-6">No timeline notes recorded yet. Use the form below to document follow-up.</p>
                      )}
                    </div>

                    {/* Add note form */}
                    <form onSubmit={handleAddNote} className="flex gap-4 items-end pt-2 border-t border-[#141414]/10">
                      <div className="flex-grow space-y-1">
                        <label className="text-[9px] font-bold uppercase tracking-widest opacity-50">Write New Note</label>
                        <textarea
                          placeholder="e.g. Call parent to schedule center tour for next Wednesday..."
                          value={newNoteText}
                          onChange={(e) => setNewNoteText(e.target.value)}
                          rows={2}
                          required
                          className="w-full px-4 py-3 bg-[#F8F7F4] border border-[#141414] rounded-xl outline-none focus:ring-2 focus:ring-brand-teal text-xs font-medium resize-none"
                        />
                      </div>
                      <button type="submit" className="px-6 py-3.5 bg-[#141414] hover:bg-brand-teal text-white rounded-xl font-kids font-bold text-xs transition-all tracking-wider uppercase">
                        Add Note
                      </button>
                    </form>
                  </div>
          ...
          {isAddAdminModalOpen && (
            <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 lg:p-0">
              ...
              <motion.div 
                ...
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.95, y: 20 }}
                className="relative w-full max-w-md bg-white rounded-none border border-[#141414] shadow-2xl p-6 md:p-10 overflow-hidden animate-in fade-in zoom-in duration-300"
              >
                <button onClick={() => setIsAddAdminModalOpen(false)} className="absolute top-6 right-6 p-2 text-[#141414] hover:bg-[#141414] hover:text-white rounded-full transition-all">
                  <X size={20} />
                </button>
                ...
                <form onSubmit={handleAddAdminSubmit} className="space-y-6">
                  <div className="space-y-1">
                    ...
                    <input 
                      type="email" 
                      required 
                      placeholder="e.g. name@auviatherapy.com" 
                      value={adminEmailForm} 
                      onChange={e => setAdminEmailForm(e.target.value)} 
                      className="w-full px-5 py-4 bg-[#F8F7F4] border border-[#141414] rounded-none outline-none font-medium font-mono" 
                    />
                    <p className="text-[9px] text-brand-sage font-medium">Invited user must sign in using Google Auth with this verified email.</p>
                  </div>
                  
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold uppercase tracking-widest opacity-50">Role</label>
                    <select 
                      value={adminRoleForm} 
                      onChange={e => {
                        setAdminRoleForm(e.target.value as any);
                        if (e.target.value !== 'center_manager') setAdminCenterForm('');
                      }} 
                      className="w-full px-5 py-4 bg-[#F8F7F4] border border-[#141414] rounded-none outline-none font-medium text-xs font-bold"
                    >
                  ...
                  {adminRoleForm === 'center_manager' && (
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold uppercase tracking-widest opacity-50">Assigned Care Center</label>
                      <select 
                        value={adminCenterForm} 
                        onChange={e => setAdminCenterForm(e.target.value)} 
                        required
                        className="w-full px-5 py-4 bg-[#F8F7F4] border border-[#141414] rounded-none outline-none font-medium text-xs font-bold capitalize"
                      >
                  ...
                  <button type="submit" className="w-full py-5 bg-[#141414] hover:bg-brand-teal text-white rounded-none font-kids font-bold text-xl transition-all shadow-xl uppercase">
                    Whitelist Email
                  </button>
                </form>
              </motion.div>
