...
export const DevelopmentalTimeline = () => {
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto py-10 px-4 sm:px-0" ref={containerRef}>
      
      {/* Background Track Line (Dashed) */}
      <div className="absolute left-[32px] sm:left-[40px] lg:left-1/2 top-0 bottom-0 w-1.5 -translate-x-1/2 border-l-[3px] border-dashed border-brand-teal/20 z-0" />
      
      {/* Animated Foreground Progress Line */}
      <motion.div 
        style={{ height: lineHeight }}
        className="absolute left-[32px] sm:left-[40px] lg:left-1/2 top-0 w-1.5 -translate-x-1/2 bg-brand-teal z-0 rounded-full origin-top"
      />

      {/* Milestone Cards */}
      <div className="space-y-12 lg:space-y-32 relative z-10">
        {milestones.map((milestone, index) => {
          ...
          return (
            <div key={index} className="flex flex-col lg:flex-row items-center w-full relative">
              ...
              {/* Center Icon Node */}
              <div className="absolute left-[32px] sm:left-[40px] lg:left-1/2 -translate-x-1/2 flex items-center justify-center">
                <motion.div 
                  initial={{ scale: 0, opacity: 0 }}
                  whileInView={{ scale: 1, opacity: 1 }}
                  viewport={{ once: true, margin: "-100px" }}
                  transition={{ type: "spring", stiffness: 100, delay: 0.2 }}
                  className={`w-12 h-12 sm:w-16 sm:h-16 rounded-full border-4 border-white shadow-xl flex items-center justify-center ${milestone.color} ${milestone.textColor} z-20`}
                >
                  {React.cloneElement(milestone.icon as React.ReactElement, { size: window.innerWidth < 640 ? 18 : 24 })}
                </motion.div>
              </div>

              {/* Right Side (Always visible on mobile, alternating on desktop) */}
              <div className={`flex-1 pl-16 sm:pl-20 lg:pl-16 w-full ${isEven ? 'lg:hidden' : ''}`}>
      ...
      <motion.div 
        ...
        className="mt-16 sm:mt-32 relative z-10 px-4 sm:px-0"
      >
        <div className="bg-brand-ink p-8 sm:p-16 rounded-[32px] sm:rounded-[40px] shadow-2xl text-center max-w-3xl mx-auto border-4 border-white relative overflow-hidden group">
          ...
          <h3 className="text-3xl sm:text-5xl font-kids font-bold text-white mb-6">
...
const TimelineCard = ({ milestone, direction }: { milestone: any, direction: 'left' | 'right' }) => {
  return (
    <motion.div 
      ...
      className={`bg-white p-6 sm:p-8 rounded-[24px] sm:rounded-[40px] shadow-lg border-2 border-transparent hover:${milestone.borderColor} transition-all duration-300 w-full group relative overflow-hidden`}
    >
      ...
      <div className="relative z-10 text-left">
        <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-4">
          <span className={`px-3 py-1.5 sm:px-4 sm:py-1.5 rounded-full ${milestone.color} ${milestone.textColor} font-kids font-bold text-[10px] sm:text-sm uppercase tracking-widest`}>
            {milestone.age}
          </span>
          <span className="text-[10px] sm:text-sm font-bold text-brand-ink/50 flex items-center gap-1">
        ...
        <p className="text-brand-sage text-base sm:text-lg font-medium leading-relaxed">
          {milestone.desc}
        </p>
      </div>
    </motion.div>
