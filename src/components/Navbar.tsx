...
export const Navbar = () => {
  ...
  const navItems = [
    ...
    },
    {
      ...
      type: 'dropdown',
      items: [
        { name: 'Parent Help Center', href: '/parent-help-center' },
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'py-4' : 'py-8'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className={`relative flex items-center justify-between px-4 sm:px-8 py-3 sm:py-4 rounded-[24px] sm:rounded-[32px] transition-all duration-500 ${isScrolled ? 'bg-white/90 backdrop-blur-xl shadow-lg border border-white/20' : 'bg-white/70 backdrop-blur-md'}`}>
                  ...
                  item.type === 'dropdown' ? (
                    <div key={item.name} className="flex flex-col">
                      <button
                        ...
                        className="flex items-center justify-between py-2 text-xl font-kids font-bold text-brand-ink text-left border-b border-gray-50"
                      >
                        {item.name}
                        <ChevronDown size={18} className={`text-brand-teal transition-transform duration-300 ${openMobileDropdown === item.name ? 'rotate-180' : ''}`} />
                      </button>
                    ...
                    <Link 
                      key={item.name} 
                      to={item.href!} 
                      className={`py-2 text-xl font-kids font-bold transition-colors border-b border-gray-50 ${location.pathname === item.href ? 'text-brand-teal' : 'text-brand-ink'}`} 
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
                      {item.name}
                    </Link>
